...
export function CartDrawer() {
  ...
  const remaining = threshold === null ? 0 : threshold - subtotal;
        ...
        {lines.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-6 px-8 text-center">
            ...
            <p className="max-w-xs text-sm text-muted">
              Pick something and it will show up here.
              {threshold !== null ? <> Free delivery once you cross {money(threshold)}.</> : null}
            </p>
            ...
            <ul className="flex-1 divide-y divide-line overflow-y-auto px-6">
              {lines.map((line) => (
                <li key={line.variantId} className="flex gap-4 py-6">
                  ...
                  >
                    <CartLineThumb line={line} className="size-24" />
                  </Link>
            ...
            <footer className="border-t border-line px-6 py-6">
              {remaining > 0 ? (
                <p className="label mb-5 text-accent">
                  {money(remaining)} more for free delivery
                </p>
              ) : (
